...
class NetworkingService {
    ...
    func fetchComments(
        ...
        onCompletion: @escaping (CommentsResult) -> Void) {
        
        func fireErrorCompletion(_ error: Error?) {
            onCompletion(CommentsResult(comments: nil, error: error,
                                          currentPage: 0, pageCount: 0))
        }
        
        var components = URLComponents(string: endpoint)
        components?.queryItems = [
            URLQueryItem(name: "query", value: "parrot"),
        ...
        task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            DispatchQueue.main.async {
                ...
                do {
                    ...
                    onCompletion(CommentsResult(comments: Array(first50),
                                                  error: nil,
